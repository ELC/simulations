...
def _run_and_summarize(config: DoubleAuctionConfig) -> tuple[RunBundle, SimulationReport]:
...
def test_double_auction_run_with_default_seed_is_deterministic(
    double_auction_fast_config: DoubleAuctionConfig,
) -> None:
    first_bundle, first_report = _run_and_summarize(double_auction_fast_config)
    second_bundle, second_report = _run_and_summarize(double_auction_fast_config)
...
def test_double_auction_export_round_trip_is_byte_identical(
    double_auction_fast_config: DoubleAuctionConfig,
) -> None:
    bundle, report = _run_and_summarize(double_auction_fast_config)
    first = serialize_run(
        feature=DOUBLE_AUCTION_FEATURE,
        config=double_auction_fast_config,
