...
) -> tuple[RunBundle, SimulationReport]:
...
def test_preferential_attachment_run_with_default_seed_is_deterministic(
    preferential_attachment_fast_config: PreferentialAttachmentConfig,
) -> None:
    first_bundle, first_report = _run_and_summarize(preferential_attachment_fast_config)
    second_bundle, second_report = _run_and_summarize(preferential_attachment_fast_config)
...
def test_preferential_attachment_export_round_trip_is_byte_identical(
    preferential_attachment_fast_config: PreferentialAttachmentConfig,
) -> None:
    bundle, report = _run_and_summarize(preferential_attachment_fast_config)
    first = serialize_run(
        feature=PREFERENTIAL_ATTACHMENT_FEATURE,
        config=preferential_attachment_fast_config,
